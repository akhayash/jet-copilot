...
- **Copilot 履歴**: 選択ディレクトリの過去の Copilot CLI セッションを表示。短命セッション（ユーザーメッセージ2件未満）は自動的に除外

### 4. ターミナル機能

[xterm.js](https://xtermjs.org/) v6 によるフルインタラクティブターミナル。URL は自動でクリック可能リンクになります。

- **再接続リプレイ**: 接続が切れても自動再接続し、直近 300 KB の出力をリプレイ。コンテキストを失いません

#### ショートカットバー（下部）

ゲームパッド風の3ブロック配置。モバイルで親指から操作しやすい:
...
| グループ | ボタン | 動作 |
|----------|--------|------|
| **方向キー**（左） | **← ↑ ↓ →** | メニュー・選択肢のナビゲーション |
| **コマンド**（中央） | **ESC** | Copilot操作のキャンセル |
| | **Mode** | Copilot CLIのモード切替（Shift+Tab送信） |
| | **Enq** | Copilot CLIに入力をキュー（Ctrl+Q送信） |
| **アクション**（右） | **↵ Enter** | 選択の確定（大きなアクセントボタン） |
...
| 🔄 **Reset** | **短押し** — ソフトリセット。**長押し（1秒以上）** — Copilot CLIをハードリスタート |

#### フローティング履歴ボタン

ターミナル右下に💬ボタンが常時表示され、現在のセッションの会話履歴にすぐアクセスできます。
...
| GET | `/api/copilot-sessions/:id/history` | セッションの会話履歴取得 |
...
│   ├── logger.js              # 構造化JSONロガー（LOG_LEVEL制御）
